    ...
    public class WebScraperService
    {
        ...
        public async Task<bool> FetchAllMissingPersonCasesAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (missingPersonsList != null)
                {
                    ...
                    await UpsertPersons("missing", missingPersonsList);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> FetchAllWantedPersonCasesAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                {
                    //now we query each valid individual and then log them to to the table
                    await UpsertPersons("wanted", wantedPersonsList);
                }
            }
            catch (Exception ex)
            {
        ...
        private static async Task<Dictionary<string, string>?> GetList(string htmlContent)
        {
            try
            {
        ...
        private async Task<bool> UpsertPersons(string typeOfPersons, Dictionary<string, string> persons)
        {
            ...
            List<CaseDTO> cases = await FetchFullCaseDetails(typeOfPersons, persons);

            //upsert into db
            try
            {
                //await _taskTrackerContext.AddRangeAsync(cases).ConfigureAwait(false);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private async Task<List<CaseDTO>> FetchFullCaseDetails(string typeOfPersons, Dictionary<string, string> persons)
        {
            List<CaseDTO> cases = [];
            ...
            foreach (var person in persons)
            {
                ...
                await _taskTrackerContext.AddAsync(@case.Adapt<CaseEntity>());
